...
enum lex_units id_type_search(sym_list * list_of_tables,lex_unit_t * name){
	...
	ht_item *act=find_item(list_of_tables,name);
...
enum lex_units tree_check(d_node * rh_node,sym_list * list_of_tables){
	...
	if(rh_node->left==NULL && rh_node->right==NULL){
		if(rh_node->type==E)
			return id_type_search(sym_list * list_of_tables,rh_node->data);
		else return ERROR;
	}
	...
	if(rh_node->right!=NULL && rh_node->left!=NULL){
		...
		if(rh_node->right->type==E && rh_node->left->type==OP){ //left side OP right side ID
			
			if(id_type_search(sym_list * list_of_tables,rh_node->right->data)== 
				tree_check(rh_node->left,list_of_tables)) 
				return id_type_search(sym_list * list_of_tables,rh_node->right->right->data);
			else
				return ERROR;
		}

		if(rh_node->left->type==E && rh_node->right->type==OP){ //left side ID right side OP
			
			if(id_type_search(sym_list * list_of_tables,rh_node->left->data)==
				tree_check(rh_node->right,list_of_tables)) 
				return id_type_search(sym_list * list_of_tables,rh_node->left->data);
			else
				return ERROR;

		}
		if(rh_node->left->type==E && rh_node->right->type==E){ //both sides ID
			if(id_type_search(sym_list * list_of_tables,rh_node->left->data)==
				id_type_search(sym_list * list_of_tables,rh_node->right->data))
				return id_type_search(sym_list * list_of_tables,rh_node->right->data);
			else
				return ERROR;
		}
	}
	...
	if(rh_node->left==NULL && rh_node->right!=NULL){
		...
		if(rh_node->right->type==E) //have to be ID 
			return id_type_search(sym_list * list_of_tables,rh_node->right->data);
		else 
			return ERROR; // tree is build incorrectly
...
unsigned Sem_analysis(d_node * node,sym_tab * main,sym_list * list_of_tables,lex_unit_t* func_name,bool should_return){
	...
	if(rh_node->right==NULL)return SYSTEM_ERROR; // no id or value to assign

	if(list_of_tables==NULL)return SYSTEM_ERROR; // no comment ;)


	enum lex_units right_sd=tree_check(node->right); // chceck derivation tree

	for(d_node * tmp=node->left;tmp!=NULL;tmp=next_left(tmp)){ //pushing left side of tree
		if(id_type_search(sym_list,tmp->data)==right_sd) // comp data types 
		else return COMPATIBLE_ERR;
	}
